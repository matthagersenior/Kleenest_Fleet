...
export async function listLiveNetworkMotifs(businessId:string,windowMinutes=60):Promise<LiveNetworkMotif[]>{
...
export function subscribeLiveNetworkMotifs(businessId:string,onChange:()=>void){
  const client=getSupabaseClient();
  let timer:ReturnType<typeof setTimeout>|null=null;
  const signal=()=>{if(timer)clearTimeout(timer);timer=setTimeout(onChange,250)};
  const channel=client.channel(`fleet-live-network-motifs-${businessId}`)
    .on('postgres_changes',{event:'*',schema:'public',table:'live_network_motif_ticks',filter:`scope_id=eq.${businessId}`},signal)
    .subscribe();
