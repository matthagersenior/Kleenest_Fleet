...
export default function FleetSignIn() {
  ...
  return <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', padding: 24 }}><View style={{ gap: 14 }}>
    ...
    <View style={{gap:6}}><Text style={{fontWeight:'800',color:'#365245'}}>Fleet email</Text><TextInput value={email} onChangeText={setEmail} autoCapitalize="none" keyboardType="email-address" autoComplete="email" textContentType="username" placeholder="Email" placeholderTextColor="#7f8d85" style={input}/></View>
    <View style={{gap:6}}><Text style={{fontWeight:'800',color:'#365245'}}>Fleet password</Text><View style={{position:'relative'}}><TextInput value={password} onChangeText={setPassword} secureTextEntry={!showPassword} autoCapitalize="none" autoCorrect={false} autoComplete="current-password" textContentType="password" placeholder="Password" placeholderTextColor="#7f8d85" style={[input,{paddingRight:76}]}/><Pressable accessibilityRole="button" accessibilityLabel={showPassword?'Hide fleet password':'Show fleet password'} onPress={()=>setShowPassword(v=>!v)} style={{position:'absolute',right:8,top:7,bottom:7,justifyContent:'center',paddingHorizontal:10,borderRadius:10,backgroundColor:'#edf3ef'}}><Text style={{fontWeight:'900',color:'#244d39'}}>{showPassword?'Hide':'Show'}</Text></Pressable></View></View>
...
const input={borderWidth:1,borderColor:'#ccd9d1',borderRadius:14,paddingHorizontal:14,paddingVertical:13,backgroundColor:'white',color:'#111827'} as const;
