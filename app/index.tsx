...
export default function FleetControlCenter() {
  ...
  return <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} />} contentContainerStyle={{ padding: 16, gap: 18, paddingBottom: 56 }}>
    <View style={{ gap: 5 }}><Text style={{ color: '#3d6754', fontSize: 13, fontWeight: '800' }}>KLEENEST FLEET · {enterprise ? 'ENTERPRISE' : 'STANDARD FLEET'}</Text><Text style={{ fontSize: 28, fontWeight: '800', color: '#12251c' }}>{name}</Text><Text style={{ color: '#5d6e65' }}>Live dispatch, execution, prevention and exception intelligence from the shared Kleenest platform.</Text></View>
