...
export default function FleetControlCenter() {
  ...
  return <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} />} contentContainerStyle={{ padding: 16, gap: 18, paddingBottom: 56 }}>
    ...
    {enterprise ? <Route href="/enterprise" title="Enterprise" detail="Multi-location Fleet control, partner networks and cross-business Enterprise state" /> : null}
...
function Route({ href, title, detail }: { href: '/operations' | '/execution' | '/signals' | '/sync' | '/intelligence' | '/metrics' | '/premium' | '/enterprise'; title: string; detail: string }) { return <Link href={href} asChild><Pressable style={{ backgroundColor: 'white', borderRadius: 18, padding: 16, gap: 5 }}><Text style={{ fontSize: 18, fontWeight: '800' }}>{title}</Text><Text style={{ color: '#63736a', lineHeight: 20 }}>{detail}</Text></Pressable></Link>; }
