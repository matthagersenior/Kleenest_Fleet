...
export default function FleetControlCenter() {
  ...
  return <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} />} contentContainerStyle={{ padding: 16, gap: 18, paddingBottom: 56 }}>
    <View style={{ gap: 5 }}><Text style={{ color: '#3d6754', fontSize: 13, fontWeight: '800' }}>KLEENEST FLEET · {enterprise ? 'ENTERPRISE' : 'STANDARD FLEET'}</Text><Text style={{ fontSize: 28, fontWeight: '800', color: '#12251c' }}>{name}</Text><Text style={{ color: '#5d6e65' }}>Live dispatch, execution, prevention, Premium distribution and exception intelligence from the shared Kleenest platform.</Text></View>
    ...
    <Route href="/operations" title="Fleet Operations" detail="Vehicles, drivers, routes, route stops, dispatch, timing, alerts, maintenance and controller policies" />
    ...
    <View style={{ backgroundColor: 'white', borderRadius: 18, padding: 16, gap: 6 }}><Text style={{ fontSize: 18, fontWeight: '800' }}>Monitoring entitlement</Text><Text style={{ color: '#5d6e65', lineHeight: 20 }}>{enterprise ? 'Enterprise Fleet monitoring is enabled for this workspace.' : 'Fleet includes one monitored location. A second monitored location requires Enterprise; Business Growth does not increase this Fleet limit.'}</Text></View>
  </ScrollView>;
}
...
function Route({ href, title, detail }: { href: '/operations' | '/intelligence' | '/premium'; title: string; detail: string }) { return <Link href={href} asChild><Pressable style={{ backgroundColor: 'white', borderRadius: 18, padding: 16, gap: 5 }}><Text style={{ fontSize: 18, fontWeight: '800' }}>{title}</Text><Text style={{ color: '#63736a', lineHeight: 20 }}>{detail}</Text></Pressable></Link>; }
