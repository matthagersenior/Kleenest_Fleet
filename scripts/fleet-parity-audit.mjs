...
for(const token of ['fleet_route_geofence_manifest','record_geofence_event','distanceMeters','expo-task-manager','expo-notifications','startGeofencingAsync','register_notification_native_push_token','enableFleetLiveNetwork','disableFleetLiveNetwork'])if(!geofence.includes(token))throw new Error(`Fleet parity: geofence/Live Network service missing ${token}`);
...
for(const token of ['Live Network','enableFleetLiveNetwork','disableFleetLiveNetwork','registerFleetPush'])if(!read('app/signals.tsx').includes(token))throw new Error(`Fleet parity: Fleet Signals missing ${token}`);
...
if(pkg.dependencies?.['expo-notifications']!=='~57.0.15'||pkg.dependencies?.['expo-task-manager']!=='~57.0.15')throw new Error('Fleet parity: Live Network notification/task dependencies must match Expo 57');
...
for(const token of ['ACCESS_BACKGROUND_LOCATION','android.permission.RECORD_AUDIO','android.permission.SYSTEM_ALERT_WINDOW','blockedPermissions','isAndroidBackgroundLocationEnabled:true','isIosBackgroundLocationEnabled:true','expo-notifications'])if(!appConfig.includes(token.replace(/\s+/g,'')))throw new Error(`Fleet parity: Live Network app config missing ${token}`);
console.log('Fleet parity audit passed with hardened Supabase-to-UI coverage across routing, Live Network, assignments, event timestamps, assets, policies, monitoring, performance, progression, metrics, Enterprise, OAuth and native runtime.');